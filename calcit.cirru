    ...
    'app.container $ %{} 'FileEntry
      :defs $ {}
        ...
        'read-grid $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn read-grid (i j grid ? size)
            let
        ...
        'row-template $ %{} 'CodeEntry (:doc |)
          :code $ quote $ def row-template
            range $ unsafe-coerce (&map:get grid-settings :size) 'Number
          :examples $ []
          :schema $ :: 'Dynamic
          ...
          :code $ quote $ defn loop-trigger! ()
            let
              ...
              dispatch! :set-grid $ -> row-template $ map
                fn (i)
                  -> row-template $ map $ fn (j)
                    iterate-next-cell i j grid rule
          :examples $ []
          :schema $ :: 'Dynamic
        'main! $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn main! () (; js/console.log PIXI)
            ...
            add-watch *store :change $ fn (store prev)
              render! (comp-container @*store) dispatch! $ {}
            reset! *loop $ js/setInterval loop-trigger! $ unsafe-coerce (&map:get grid-settings :interval) 'Number
            println "|App Started"
          :examples $ []
          :schema $ :: 'Dynamic
